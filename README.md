...
- [WorkBuddy Experts](https://github.com/vbarter/workbuddy-experts) - 提示词工程学习参考资产集，整理 WorkBuddy 4.22.16 的 246 位专家提示词、7 个 Nunjucks 系统模板、2 个内置 Skill 和客户端逆向报告；仓库明确说明 MIT 仅覆盖整理/索引/报告，专家正文与腾讯定制模板仍受上游及腾讯版权约束，并限个人学习、禁止批量公开转载或商业再分发，使用前请核对 NOTICE、版本、版权和内容合规边界.
...
- [CareerStar（职途星）](https://github.com/HanGu007/workbuddy-career-skills) - MIT 许可的 WorkBuddy 求职专家包，覆盖简历诊断/改写、面试辅导、企业信息查询、HTML 简历和投递追踪；会处理简历与就业资料，并可连接 IMA、企查查等外部服务，使用前请脱敏并核对授权、数据流和背调结论.
- [WorkBuddy Skin Skill](https://github.com/zhangxiaoqiang1991/workbuddy-skin-skill) - MIT 许可、可直接下载 ZIP 的 WorkBuddy 换肤 Skill，支持上传参考图生成私有主题、10 套内置 Skin、截图验证和失败恢复；主题默认只保存在本机，使用时请确认图片权利、联网研究范围和本地 CDP 权限.
...
- [WeChat Article Skills](https://github.com/aiworkskills/wechat-article-skills) - Apache-2.0 许可、兼容 WorkBuddy 的公众号运营 Skills，覆盖选题、写作、审稿、排版、配图和草稿发布，并提供 WorkBuddy 组合教程；安装会接触本地配置、图片/文章资料及可能的外部模型或发布服务，使用前请确认账号授权、内容版权、密钥保存和发布范围.
...
- [CodeDrobe Skills](https://github.com/CodeDrobe/skills) - Apache-2.0 的可安装主题 Skill，明确支持 Tencent WorkBuddy，提供主题创建、应用、验证、修复、恢复和发布流程；它会通过本地 CDP 连接目标桌面应用，应用主题可能需要启动或重启应用，使用前请确认参考图片、应用权限和重启影响.
- [Data Analysis Skills](https://github.com/cabbage2000-lab/data-analysis-skills) - MIT 许可、已说明 WorkBuddy 适配和实测的多平台数据分析 Skill，可将 CSV、Excel、JSON、TSV 整理为带洞察的单文件 HTML 报告，并提供行业模板、评测和测试；它会读取数据并可能加载 CDN 图表资源，使用前请脱敏并核对输出与网络范围.
...
- [Travel Planner Skill](https://github.com/ycyliu/travel-planner-skill) - MIT 许可、直接兼容 WorkBuddy/CodeBuddy 的旅行规划 Skill，通过需求确认、外部搜索和可选小红书 MCP 生成响应式 HTML 攻略与三档预算；可能接触社交平台登录态、个人行程和动态价格，使用前请保护会话、核对来源/余票/房态/预约渠道，并人工确认最终方案.
...
- [DSH WorkBuddy XD Pool](https://github.com/aosi526/dsh-workbuddy-xdpool) - MIT 许可的 DeepSeek Harness 插件，将 WorkBuddy 桌面端历史登录账号自动发现并合并为可故障转移的模型池，提供模型目录、额度、冷却和回环流式桥；它会扫描并保存多账号登录快照、自动轮换 Token，并让请求跨账号切换，安装前请确认每个账号的授权、凭据目录、模型计费、端口和平台条款，勿用于共享或转售.
...
- [QClaw × WorkBuddy Bridge](https://github.com/liuboacean/qclaw-workbuddy-bridge) - MIT 许可的 macOS 工作流桥，通过共享 JSON 队列和 launchd 事件把微信/QClaw 任务交给 WorkBuddy 执行，并提供任务提交、结果查询和队列管理三个 Skill；启用前请确认队列内容、launchd 权限和无人值守执行范围.
...
- [Maestro](https://github.com/joosure/Maestro) - AGPL-3.0 的真实项目任务编排平台，支持将 Linear/TAPD 任务连接到 GitHub/CNB 和 Codex、Claude Code、OpenCode、CodeBuddy，按隔离工作区执行、记录 diff/日志/工具调用并回写任务结果；提供 Memory/Mock 本地流程、中文文档、容器安全扫描和 CodeBuddy 专用镜像。真实工作流可能克隆仓库、运行 Agent、推送分支、创建 PR/MR、更新任务和评论，首次使用应从 mock 开始，使用低权限凭据、一次性工作区并逐项审查模板和网络范围.
...
- [CodeBuddy Statusline](https://github.com/runzhi/codebuddy-statusline) - MIT 许可、跨平台的 CodeBuddy Code 状态栏，显示上下文进度、Token、工具调用、费用、Credits、耗时与代码增删，并提供详细成本报告和可配置布局；安装脚本会写入 `~/.codebuddy/settings.json`、创建缓存、链接命令，并在 git-clone 模式下每日尝试后台更新，运行时会读取会话 transcript，执行前请审查配置写入、会话隐私、更新来源和缓存范围.
...
- [CodeBuddy Proxy](https://github.com/wnddd839/codebuddyapi-proxy) - BSD-3-Clause 的 Go 自托管协议网关，将 CodeBuddy OAuth/API 接入转换为 OpenAI 兼容接口，支持流式请求、模型发现、国内/国际端点、账号池、回环管理台、源码构建和多平台发布；默认回环并说明凭据文件 `0600`、API Key 和恢复流程，但 OAuth、账号轮换、管理台和预编译包仍需按授权账号、网络边界、日志/备份和服务条款逐项复核.
...
- [Better Harness](https://github.com/QoderAI/better-harness) - MIT 许可的 Agent Harness 工程平台，明确支持 WorkBuddy，提供证据化工作流报告、可恢复任务、Skill/MCP/Hook 边界、受控实验和 37 个 Agent 平台适配；WorkBuddy 当前没有托管插件生命周期，安装与分析会产生项目配置、报告和状态文件，使用前请审查目标路径、会话范围与外部模型连接.
