...
1. 在[在线目录](https://sandbaseai.github.io/awesome-workbuddy/)搜索你的目标，打开原项目 README。
2. 确认 WorkBuddy 版本、系统要求、许可证和依赖，再用小任务试用。
3. 按原项目说明安装：Skill 通常上传 ZIP 或复制到 Skills 目录；MCP 通常在设置中添加连接器；工作流或教程通常直接复制提示词或文档。
4. 安装后重新打开会话，并在任务中明确写出要使用的 Skill 或 MCP 名称。

如果原项目没有写清安装方法，或要求提交密钥、Cookie、客户数据，先不要安装。
